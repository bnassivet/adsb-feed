...
| Agents "do not start": process alive, port not listening, **zero-byte log** | MLflow is down. Both agents resolve their tracing experiment at import time and retry a refused connection with backoff, so they hang before writing a line. `make doctor` reports it; start MLflow, or set `agents.mlflow_uri = ""`. |
| Agent ignores a setting in `rust/adsb-agent/.env` | `stack.sh` exports `ADSB_AGENT_*` from `[agents]`, and **environment beats `.env`** in pydantic-settings. Anything the stack config covers (`llm_base_url`, `mlflow_uri`, the ports, the tool-server and sim-agent URLs) must be changed in the stack file, not the `.env`. |
| Chat fails with `400 No models loaded ... use 'lms load'` | The LLM endpoint is up but no model is *loaded* — `/v1/models` lists **downloaded** ones, so reachability alone looks fine. `make doctor` now distinguishes the two. |
| Chat fails with a model-not-found style error | The id must match the endpoint. `agents.llm_base_url` should be the **MLflow AI Gateway** (`http://localhost:5010/gateway/mlflow/v1`), which resolves the short endpoint name (`gemma-4-12b-qat`) and loads on demand. Pointed straight at LM Studio you need the full id (`google/gemma-4-12b-qat`) *and* a model already loaded. |
